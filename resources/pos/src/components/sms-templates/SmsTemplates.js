...
import {
    fetchSmsTemplates,
    activeInactiveSms,
} from "../../store/action/smsTemplatesAction";
...
const SmsTemplates = (props) => {
    ...
    const goToEditProduct = (item) => {
        const id = item.id;
        navigate(`/admin/pos/sms-templates/${id}`);
    };
    ...
    const itemsValue =
        ...
        smsTemplates.map((smsTemplate) => ({
            ...
            is_active:
                smsTemplate?.attributes?.status === 0 ||
                smsTemplate?.attributes?.status === false
                    ? false
                    : true,
            id: smsTemplate.id,
        }));
    ...
    return (
        <MasterLayout>
            ...
            <ReactDataTable
                ...
                to="#/admin/pos/sms-templates/create"
            />
        </MasterLayout>
    );
};
