...
const SmsTemplateForm = (props) => {
    ...
    const disabled =
        singleSMSTemplate &&
        singleSMSTemplate[0].content === smsTemplateValue.content;
    ...
    const handleValidation = () => {
        ...
        if (!smsTemplateValue["name"]) {
            ...
            errorss["content"] = getFormattedMessage(
                "sms-content.error.message"
            );
        } else if (smsTemplateValue["content"].trim().length >= 300) {
            errorss["content"] = getFormattedMessage(
                "sms-content-text.error.message"
            );
        } else {
            isValid = true;
        }
