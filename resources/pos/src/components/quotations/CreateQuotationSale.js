...
const CreateQuotationSale = (props) => {
    ...
    const selectedStatus =
        ...
        quotations.attributes.status &&
        status.filter((item) => item.value === quotations.attributes.status);

    const itemsValue = quotations &&
        quotations.attributes && {
            ...
            sale_items: quotations.attributes.quotation_items.map((item) => ({
                ...
                stock:
                    ...
                    item.product.stocks.filter(
                        (item) =>
                            item.warehouse_id ===
                            quotations.attributes.warehouse_id
                    ),
    ...
    return (
        <MasterLayout>
            <TopProgressBar />
            <HeaderTitle
                title={getFormattedMessage("sale.create.title")}
                to="/admin/app/quotations"
            />
            {isLoading ? (
                <Spinner />
            ) : (
