...
const Suppliers = (props) => {
    const { fetchSuppliers, suppliers, totalRecord, isLoading, allConfigData } =
        props;
    ...
    const goToEditProduct = (item) => {
        const id = item.id;
        navigate(`/admin/pos/suppliers/edit/${id}`);
    };
        ...
        suppliers.length >= 0
            ? suppliers.map((supplier) => ({
                  date: getFormattedDate(
                      supplier.attributes.created_at,
