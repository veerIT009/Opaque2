...
import settingsFormValidate from "./settingsFormValidate";
...
const SettingsForm = (props) => {
    ...
    const [groupText, setGroupText] = useState(
        initialValues.currency ? initialValues.currency.symbol : null
    );
    ...
    const [logo, isDefaultLogo, logoFile, onLogoChange, onRemoveLogo] =
        imagePicker(
            change,
    ...
    const onSave = (formValues) => {
        ...
        const settings = [
            ...
            prepareFormData(
                settingsKey.LIBRARY_NAME,
                library_name,
                settingsDisplayName.APP_NAME
            ),
            prepareFormData(
                settingsKey.PENALTY_PER_DAY,
                penalty_per_day,
                settingsDisplayName.PENALTY_PER_DAY
            ),
            prepareFormData(
                settingsKey.ISSUE_DUE_DAYS,
                issue_due_days,
                settingsDisplayName.ISSUE_DUE_DAYS
            ),
            prepareFormData(
                settingsKey.RETURN_DUE_DAYS,
                return_due_days,
                settingsDisplayName.RETURN_DUE_DAYS
            ),
            prepareFormData(settingsKey.LANGUAGE, language.id, language.name),
            prepareFormData(
                settingsKey.RESERVE_BOOKS_LIMIT,
                reserve_books_limit,
                settingsDisplayName.RESERVE_BOOKS_LIMIT
            ),
            prepareFormData(
                settingsKey.ISSUE_BOOKS_LIMIT,
                issue_books_limit,
                settingsDisplayName.ISSUE_BOOKS_LIMIT
            ),
            prepareFormData(
            ...
            prepareFormData(
                settingsKey.STRIPE_KEY,
                stripe_key,
                settingsDisplayName.STRIPE_KEY
            ),
            prepareFormData(
                settingsKey.STRIPE_SECRET,
                stripe_secret,
                settingsDisplayName.STRIPE_SECRET
            ),
        ];
    ...
    const logoPickerOptions = {
        image: initialValues.library_logo
            ? initialValues.library_logo
            : publicImagePath.APP_LOGO,
    ...
    const faviconPickerOptions = {
        image: initialValues.library_favicon
            ? initialValues.library_favicon
            : publicImagePath.APP_LOGO,
    ...
    return (
        <Row className="settings">
            <Col xs={2} className="settings__logo">
                <h6 className="settings__logo-heading">
                    {getFormattedMessage("image-picker.dropdown.logo.label")}
                </h6>
                <div>
                    <Field
                        name="file_name"
                ...
                <h6 className="settings__favicon-heading mt-3">
                    {getFormattedMessage("image-picker.dropdown.favicon.label")}
                </h6>
                <div>
                    <Field
                        name="file_name"
            ...
            <Col xs={2} className="settings__favicon mt-2"></Col>
            <Col xs={12}>
                <SaveAction
