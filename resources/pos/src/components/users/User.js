...
const User = (props) => {
    ...
    const itemsValue =
        ...
        users.map((user) => ({
            ...
            role_id: user.attributes.role.map((ro) => ro.id),
    ...
    const goToEdit = (item) => {
        const id = item.id;
        window.location.href = "#/admin/pos/users/edit/" + id;
    };
        ...
        },
        {
            ...
            cell: (row) => (
                <ActionButton
                    ...
                    isEditMode={row.role_id[0] == 1 ? false : true}
                    isDeleteMode={row.role_id[0] == 1 ? false : true}
