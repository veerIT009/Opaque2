...
import { editTestimonial } from "../../admin/store/actions/testimonialAction";

const prepareInitialValue = (testimonial) => {
    const { id, name, occupation, description, image, image_path } =
        testimonial;
    return {
        id,
        name,
...
const EditTestimonial = (props) => {
    ...
    return (
        <Modal
            {...props}
            content={<TestimonialForm {...prepareFormOption} />}
        />
    );
};
