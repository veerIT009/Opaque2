...
const EditPurchaseReturn = (props) => {
    ...
    const selectedStatus =
        ...
        purchaseReturn.attributes.status &&
        status.filter(
            (item) => item.value === purchaseReturn.attributes.status
        );
    const supplierId =
        purchaseReturn &&
        purchaseReturn.attributes &&
        purchaseReturn.attributes.supplier_id;
    const warehouseId =
        purchaseReturn &&
        purchaseReturn.attributes &&
        purchaseReturn.attributes.warehouse_id;
    const supplier =
    ...
    const warehouseName =
        warehouse[0] && warehouse[0].attributes && warehouse[0].attributes.name;

    purchaseReturn &&
        purchaseReturn.attributes &&
        purchaseReturn.attributes.purchase_return_items.forEach((item) => {
    ...
    const itemsValue = purchaseReturn &&
        purchaseReturn.attributes && {
            ...
            purchase_return_items: editPurchaseReturnArray(
                purchaseReturn.attributes.purchase_return_items,
                purchaseReturn.attributes.warehouse_id
            ),
            newItem: "",
    ...
    return (
        <MasterLayout>
            <TopProgressBar />
            <HeaderTitle
                title={getFormattedMessage("purchase.return.edit.title")}
                to="/admin/app/purchase-return"
            />
