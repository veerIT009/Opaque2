...
const EditRole = (props) => {
    const { roles, fetchRole, fetchPermissions, isLoading, permissions } =
        props;
    const { id } = useParams();
    ...
    const newPermission =
        roles[0] &&
        roles[0].attributes &&
        preparePermissions(permissions, roles[0].attributes.permissions);

    return (
        <MasterLayout>
            <TopProgressBar />
            <HeaderTitle
                title={getFormattedMessage("role.edit.title")}
                to="/admin/pos/roles"
            />
            {isLoading ? (
                <Spinner />
...
export default connect(mapStateToProps, { fetchRole, fetchPermissions })(
    EditRole
);
