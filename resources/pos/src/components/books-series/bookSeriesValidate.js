...
export default (formValues) => {
    const errors = {};
    if (!formValues.title) {
        errors.title = getFormattedMessage(
            "books-series.input.title.validate.label"
        );
    }
    ...
    if (formValues.series_items && formValues.series_items.length) {
        formValues.series_items.forEach((item, index) => {
            const bookErrors = {};
            if (!item || !item.book) {
                bookErrors.book = getFormattedMessage(
                    "books-series.items.select.book-name.validate.label"
                );
