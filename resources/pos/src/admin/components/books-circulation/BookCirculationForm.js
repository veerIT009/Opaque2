...
const BookCirculationForm = props => {
    const {
        ...
        clearAvailableBookLimit, toggleDueBookModal, hasPenaltyCollected, penaltyPreDays, selectedCurrency
    } = props;
    ...
    const renderLateFeeInputs = (status) => {
        ...
        return (
            <Row>
                ...
                <Col xs={12} sm={6}>
                    {hasPenaltyCollected ?
                        <Field name='collected_penalty' type="number" isDefaultCurrency={true}
                               label="books-circulation.input.amount.label" min="0"
                               groupText={selectedCurrency} component={InputGroup}/> : ''
                    }
                </Col>
            </Row>
        )
