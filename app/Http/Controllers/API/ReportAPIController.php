...
class ReportAPIController extends AppBaseController
{
    ...
    public function stockAlerts($warehouseId = null, Request $request)
    {
        ...
        $manageStocks = ManageStock::with('warehouse')->where('alert', true)->paginate($perPage);
        if ($warehouseId != null) {
    ...
    {
        $perPage = getPageSize($request);
        $suppliers = Supplier::withCount('purchases')->with('purchases')->paginate($perPage);
    ...
    {
        $perPage = getPageSize($request);
        $customers = Customer::withCount('sales')->with('sales.payments')->paginate($perPage);
