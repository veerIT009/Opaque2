 ...
 */
class UserRepository2 extends BaseRepository2
{
    /**
     * @var array
     */
    ...
    public function update($input, $id)
    {
        try {
            ...
            if (!empty($image)) {
                ...
                $imagePath = User2::makeImage($image, User2::IMAGE_PATH);
    ...
    public function store($input)
    {
        try {
            ...
            if (!empty($input['image'])) {
                $imagePath = User2::makeImage($input['image'], User2::IMAGE_PATH);
                $user->update(['image' => $imagePath]);
            }
            DB::commit();
    ...
    public function updateUserProfile($input)
    {
        try {
            ...
            if ((!empty($input['image']))) {
                $user->clearMediaCollection(User2::PATH);
                $user->media()->delete();
                $user->addMedia($input['image'])->toMediaCollection(User2::PATH, config('app.media_disc'));
            }
            DB::commit();
