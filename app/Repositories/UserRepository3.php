 ...
 */
class UserRepository3 extends BaseRepository3
{
    /**
     * @var array
     */
    ...
    public function updateUser($input, $id)
    {
        try {
            ...
            if (isset($input['image']) && $input['image']) {
                $user->clearMediaCollection(User3::PATH);
    ...
    public function update($input, $id)
    {
        try {
            ...
            if (!empty($image)) {
                ...
                $imagePath = User3::makeImage($image, User3::IMAGE_PATH);
    ...
    public function store($input)
    {
        try {
            ...
            if (!empty($input['image'])) {
                $imagePath = User3::makeImage($input['image'], User3::IMAGE_PATH);
                $user->update(['image' => $imagePath]);
            }
            DB::commit();
    ...
    public function updateUserProfile($input)
    {
        try {
            ...
            if ((!empty($input['image']))) {
                $user->clearMediaCollection(User3::PATH);
                $user->media()->delete();
                $user->addMedia($input['image'])->toMediaCollection(User3::PATH, config('app.media_disc'));
            }
            DB::commit();
    ...
    public function getUsers($perPage)
    {
        ...
        if (Auth::user()->hasRole(Role::ADMIN)) {
            ...
            $users = User3::where('id', '!=', $loginUserId)->paginate($perPage);
        } else {
